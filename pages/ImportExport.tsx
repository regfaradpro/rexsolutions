...
import { Car, HardHat, Utensils, ArrowRight } from 'lucide-react';

type Sector = 'vehicules' | 'materiaux' | 'alimentation';
...
const ImportExport: React.FC = () => {
  const sectors = [
    { 
      ...
      desc: 'Importation de véhicules d\'exception, utilitaires et logistique automobile spécialisée.',
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="mb-24 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <p className="text-xl text-gray-500 max-w-2xl font-light leading-relaxed">
              Une expertise logistique globale pour vos échanges stratégiques. Cliquez sur un pôle pour accéder aux détails et au formulaire de commande.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            {sectors.map((sector, idx) => (
              <Link
                ...
                className="group relative h-[600px] rounded-[3.5rem] overflow-hidden text-left transition-all duration-700 hover:-translate-y-4 shadow-2xl hover:shadow-black/20 block animate-in fade-in slide-in-from-bottom-10"
              ...
              >
                ...
                <div className="absolute inset-0 p-12 flex flex-col justify-end">
                  <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center mb-8 transform group-hover:scale-110 group-hover:bg-black group-hover:text-white transition-all duration-500">
                    {sector.icon}
                  </div>
                  <h3 className="text-4xl font-black text-white uppercase tracking-tighter mb-4">{sector.label}</h3>
                  <p className="text-gray-300 font-light text-lg leading-relaxed mb-8 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                    {sector.desc}
                  </p>
                  <div className="flex items-center gap-3 text-white font-black text-xs uppercase tracking-[0.3em]">
